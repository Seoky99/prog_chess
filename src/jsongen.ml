let position_maker height width r c white is_last =
  ...
  (*Looks ugly you can't pattern match against constants w/ variable
    expr :p *)
  let piece =
    if r = 2 then "black_pawn"
      ...
      let start_pos =
        if board_even then ((width - 8) / 2) + 1
        else ((width - 8 - 1) / 2) + 1
      in
...
(** [create_board n r c max_col] Creates an [n] x [n] chess board json,
    starting at position ([r],[c]) with the template: { "name": [r, c],
    "positions": [ { 'id': (rxc), 'obstacle': 'none or some' 'color':
    'white or black' }, etc. ] } Note: single string markers will be
    replaced by double quotes. *)
let rec create_board_helper height width r c sqrn =
  if sqrn = 0 then ""
  else
    let is_even = (r + c) mod 2 = 0 in
    let is_last = r = height && c = width in

    if c = width then
      position_maker height width r c is_even is_last
      ^ create_board_helper height width (r + 1) 1 (sqrn - 1)
    else
      position_maker height width r c is_even is_last
