...
let rec pos_helper board num =
  if num = 0 then [] else n_row num board :: pos_helper board (num - 1)

let position_board board = pos_helper board (num_rows board)

(*let tuple_printer t = match t with | (x,y) -> "(" ^ (string_of_int x)
  ^ "," ^ (string_of_int y) ^ ")" *)

let piece_of_position pos lst =
  (* let () = print_endline (tuple_printer (id_from_position position))
     in let () = print_endline (tuple_printer (List.hd (nth_elt (fst
     pos) lst)).id) in *)
...
(* let tuple_to_str t = match t with | (x,y) -> "(" ^ (string_of_int x)
   ^ "," ^ (string_of_int y) ^ "), "

   let rec position_printer plst = match plst with | [] -> "" | h :: t
   -> match h with | {id; _} -> tuple_to_str id ^ position_printer t *)
...
let put_piece id piece board =
  ...
  (* let () = print_endline (string_of_int (fst position_at_id.id)) in
     let () = print_endline (string_of_int (snd position_at_id.id))
     in *)
  let new_position =
    replace_element
      (n_row (fst id) board)
      (snd id)
      { position_at_id with piece }
  in

  (*let () = print_endline (tuple_printer(id_pos_lst (n_row (fst id)
    board))) in let () = print_endline (tuple_printer(id_pos_lst
    new_position)) in *)
  let new_2dlst =
...
(*match piece_from_board with | Nothing -> {board with positions =
  new_2dlst} | _ -> failwith "Piece already exists there"*)
